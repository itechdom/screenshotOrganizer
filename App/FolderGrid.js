              ...
              <List>
                {folderList.map((folder, index) => {
                  return <ListItem button key={index} onPress={() => { this._onSelectFolder(folder) }}>
                    {
                      (folder.thumbnail) ? <Thumbnail square style={{ width: 50, height: 50, resizeMode: 'contain' }} size={80} source={folder.thumbnail} /> : <Thumbnail square size={80} source={require('../img/empty-box.png')} />
                    }
                    <Body>
                      <Text>{folder.title}</Text>
                      <Text note>{(folder.screenshotList) ? folder.screenshotList.length : 0} Photos</Text>
                    </Body>
                    <Right>
